...
Each microphone arm is oriented at an angle of $45^\circ$ to the center pole.
\begin{figure}[h!]
	\centering
...
The microphone arms can be fully extended to achive a planar array geometry or folded towards the center pole to a minimum angle of $90^\circ$.
...
The integrated temperature sensor features a measurement accuracy of ±$\,0.3\,^\circ$C in the range of $0\,-\,65\,^\circ$C.
...
Consequently, heat-emitting components on the board affect the \textit{BMP388}'s temperature readings, leading to a temperature offset of approximately $2.5\,^\circ$C.
...
In situations where \acrshort{gnss} data is not available, the system falls back to the \acrshort{rtc} time.
While this ensures continuous timekeeping, the \acrshort{rtc}'s limitation to second-level resolution presents a challenge.
It does not provide detailed information about the exact moment whenever a second has passed.
...
It is based on the acquisition system's \acrshort{gui} and makes again use of the \acrshort{lvgl} framework.
...
\begin{minipage}{\linewidth}
	...
	When the arms are fully unfolded (horizontal) the angle is $0.0^\circ$.
	When the arms are fully folded towards the centre pole the angle is $90.0^\circ$.
...
It runs on a external device, in this case a \textit{Dell Latitude 5310} Laptop
with \textit{Ubuntu} installed.

\subsection{Audio Interface}
The Audio Interface provides the application with the audio data
from either the \acrshort{tcp} stream or a pre recorded WAV File.
...
The \acrshort{tcp} streamer allows to stream the audio directly via \acrshort{tcp} form the microphone array.
...
The WAV stream is made with the \textit{PyAudio} and the \textit{WAVE} library from python.
With the \textit{WAVE} library a \texttt{.wav} file is opened and then
played back with \textit{PyAudio}.
Via a callback in the played audio is also stored into the ring buffer.
On \textit{Windows} and \textit{macOS} \textit{PyAudio} could not stream a 32 channel WAV file.
Hence the application needs to be run on \textit{Ubuntu}.
...
It continuously reads the \acrshort{json} containing various data
...
and an array configuration. To open a \acrshort{tcp} connection, the \acrshort{ip} address and port
of the array is needed.
Connections can also be closed with the \textit{Disconnect} button.
...
If no \acrshort{gnss} data is available the default position is by the
building 8 at \acrshort{ost} in Rapperswil.
...
If the external system does not have an internet connection to display the map,
