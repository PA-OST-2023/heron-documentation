...
Uising the control sequence the recieved data is preprocessed and then stored into 
the buffer.
...
To retrieve the sensor datas from the microphone array
a comminucator class was created.
It continuously reads the JSON containing various data 
via a POST request from the microphone array.

\subsection{Tracker}
The tracker consists of three major parts, the beamformer, peak detector and 
tracker.
\subsubsection*{Beamforming}
First a data block is ran through the beamforming filterbank.
Contrary to prior beamforming grid, a pseudo unformly sampled 
...
If only one source needs to be tracked, a simple argmax on the beamformer 
response is sufficient.

For the case of multiple sources a more elaborate peakdetector is used.
To simplify the peakdetector, the beamformer respones, wich is defined on
a sphere is maped onto a flat surface with
\begin{align}
	x &= \theta \cos\phi \\
	y &= \theta \sin\phi.
	\label{eq:maping}
...
median blur filter.
The resulting peaks can be further filtered by minimal hieight and 
...
\subsection{GUI}
To visualize the beamforming responses and control the tracker a web based GUI with 
Plotly was created.
Figure \ref*{fin:fig:gui} shows a screenshot of the GUI while tracking is performed
on e pre recorder WAV file.
\begin{figure}
	\centering
...
\subsubsection*{Beamforming}
The beamformer response is shown in two different ways.
The top plot shows the mapped beamformer response in a surface plot, where
...
for the tracks to be displayed in the right compass diraction.
