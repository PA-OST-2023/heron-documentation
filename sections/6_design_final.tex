...
The Proportional (P) component of the \acrshort{pi}-controller is set to zero to avoid amplifying the \acrshort{rtc}'s abrupt second changes, which could lead to output irregularities.
...
This configuration allows for a gradual accumulation of timing errors, rather than responding to immediate changes, leading to a smoother and more consistent output.
A resulting integral gain of 0.01 was found to provide the best results.
...
The steering angle grid used in Chapter \ref*{chap:aev} has the drawback
that for some steering angle much more points are calculated than for others.
To reduce the amount of calculations a new angle grid was created, based on
a pseudo uniformly sampled semisphere.
