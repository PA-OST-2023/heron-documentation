...
\begin{equation}
  \alpha_n = 
  \frac{r_{m_n} \cos(\varphi_s - \varphi_{m_n}) \omega}{c}.
\end{equation}
...
This introduces the angle $\theta$ as the inclination.
\todo{End this.} 

\subsubsection{Delay and Sum Beamformer}
The basic beamformer is the delay and sum beamformer.
As the name already tells, it delays the measured signals with the steering
vector and then sums the resulting signals \dots
\begin{figure}
  \centering
  %    \includegraphics[width=0.25\textwidth]{mesh}
  \includegraphics[]{radial_f.pdf}
  \caption{BalaBula}
  \label{ssl:fig:gurke}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{A selection of the hyperbolas resulting from the time-delays $t_{m,n}$.}
		\label{ssl:fig:gurke2}
	\end{subfigure}
	\caption{Near field \acrshort{ssl} with a linear microphone Array.
		the  are distributed on a straight line a degree of freedom is lost and
		$\bm{S}$ mirrored at this line is also a possible solution.}
	\label{fig:gurken}
\end{figure}
