...
\begin{figure}[h]
	...
	\caption{$G(\phi, \theta)$ for $-180^\circ \leq \phi \leq 180^\circ$ and $0^\circ \leq \theta \leq 90^\circ$.
...
The \acrfull{pap} ratio is the ratio between the peak power and
...
With these considerations the \acrshort{pap} ratio is defined as
\begin{equation}
	PAP = \frac{\max \, P(\phi, \theta)}{\sum_{\phi, \theta}^{} P(\phi, \theta) dA(\phi, \theta)}.
\end{equation}
A high \acrshort{pap} ratio is desired, since it means that little power is leaking
...
Figure \ref{aev:fig:MicCirc} shows how the \acrshort{pap} ratio changes when
more microphones are added to an array with a fixed radius.
For smaller arrays, the best possible \acrshort{pap} is reached with less
...
the change of the main lobe area and \acrshort{pap} ratio can be seen for
...
sampled better.
Also the \acrshort{pap} ratio becomes better with an increase of the radius.
...
\begin{figure}[h]
	...
	\caption{Main lobe area ratio of circular arrays with different radius.}
...
\begin{figure}[h!]
	...
	\caption{A Multi circular array that consists of four circular arrays
...
$\approx 2.1\%$ an the \acrshort{pap} ratio is between 14 and 16.
Comparing this to the multicircular array a \acrshort{pap} ratio of up
...
\end{minipage}
\vspace{1.4cm}    % Adjust this depending on the length of the text above
...
To simulate a drone the sound of a \textit{DJI Mavic Pro} drone was played back on
\textit{UE BOOM} loudspeakers.
Various angles were tested by turning and tilting the array such that
the loudspeakers are in the direction to be tested.
The distance of the loudspeaker to the array was set to 15\,m.
...
\begin{table}[h]
	...
	\begin{tabular}{ |c"c|c|c| }
		...
		\makecell{0.3\,m}         &
		\makecell{$\phi_o = 11.25^\circ$}             \\
		\hline
		Multi circ 2              &
		\makecell{Mutli-circular} &
		\makecell{0.3\,m}         &
		\makecell{$\phi_o = 22.5^\circ$}              \\
		\hline
		Arch                      &
...
To reduce this a \textit{RØDE DeadWombat} microphone cover was cut into small pieces
...
\begin{figure}[ht]
	...
	\begin{subfigure}[b]{0.49\textwidth}
		\centering
		\includegraphics[width=0.93\textwidth]{images/5_array_evaluation/array_test_2.jpg}
		\caption{Multi circular array}
...
The measured data was evaluated using the main lobe area and \acrshort{pap} ratio.
...
\begin{figure}[ht]
	...
	\begin{subfigure}[b]{1\textwidth}
		...
		\label{fig:comp2}
	\end{subfigure}
	\caption{Measurements and simulation comparisons with a a source located at $(r, \phi, \theta) = (15\,\text{m}, 0, \theta)$.
...
From the microphone to the \textit{Teensy 4.1} the signals is dependent on different clocks and
...
To find a suited $r_i$, the main lobe area and \acrshort{pap} ratio was calculated for
various $r_i$.
The results are depicted in Figure \ref{fig:finflat}.
Figure \ref{fig:finpap} shows that the \acrshort{pap} does not
...
main lobe area and \acrshort{pap} ratio.
